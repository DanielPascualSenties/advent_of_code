...
def move_down(sea_map):
    robot_row, robot_column = find_robot(sea_map)
    for i in range(robot_row + 1, 100):
        if sea_map[i][robot_column] == ".":
            sea_map[i][robot_column] = "O"
            sea_map[robot_row][robot_column] = "."
            sea_map[robot_row + 1][robot_column] = "@"
            print("Moved down")
            return sea_map
        if sea_map[i][robot_column] == "O":
            pass
        if sea_map[i][robot_column] == "#":
...
def move_robot(sea_map, move):
    ...
    match move:
        ...
        case "v":
            print(f"Moving down")
            return move_down(sea_map)
        case "<":
            print(f"Moving left")
            return move_left(sea_map)
        case "^":
            print(f"Moving up")
            return move_up(sea_map)
        case _:
...
if __name__ == '__main__':
    print("Running main")
    result = day_15(input_name="day15_input")
    print(result)
